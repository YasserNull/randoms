...
package com.yn.tests.hvscrollview;

import com.yn.tests.R;
...
//    private boolean mIsBeingDragged = false;
    ...
    public static final int SCROLL_ORIENTATION_GRID = 4;
    public static final int SCROLL_ORIENTATION_HORIZONTAL_VERTICAL = 5;
    private int mScrollOrientation;
    private int mLockedScrollAxis = SCROLL_ORIENTATION_NONE;
    ...
    public boolean canScrollHorizontally() {
        if (mScrollOrientation == SCROLL_ORIENTATION_GRID || mScrollOrientation == SCROLL_ORIENTATION_HORIZONTAL_VERTICAL) {
            return true;
        }
    ...
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed) {
        if (!consumed) {
            float xvel = velocityX;
            float yvel = velocityY;
            if (mScrollOrientation == SCROLL_ORIENTATION_GRID) {
                if (Math.abs(xvel) > Math.abs(yvel)) {
                    yvel = 0;
                } else {
                    xvel = 0;
                }
            } else if (mScrollOrientation == SCROLL_ORIENTATION_HORIZONTAL_VERTICAL) {
                // For a nested fling, the drag happened in a child, so mLockedScrollAxis is not set
                // for this view's gesture. We determine the axis from the velocity itself.
                if (Math.abs(xvel) > Math.abs(yvel)) {
                    yvel = 0;
                } else {
                    xvel = 0;
            ...
            return fling((int) xvel, (int) yvel);
        }
        return false;
    }
...
//        }
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                */
                ...
                if (mScrollState != SCROLL_STATE_DRAGGING) {
                    ...
                    if (mScrollOrientation == SCROLL_ORIENTATION_HORIZONTAL_VERTICAL) {
                        if (Math.abs(dx) > mTouchSlop || Math.abs(dy) > mTouchSlop) {
                            startScroll = true;
                            if (Math.abs(dx) > Math.abs(dy)) {
                                mLockedScrollAxis = SCROLL_ORIENTATION_HORIZONTAL;
                                mLastMotionX = mInitialTouchX + mTouchSlop * (dx < 0 ? -1 : 1);
                            } else {
                                mLockedScrollAxis = SCROLL_ORIENTATION_VERTICAL;
                                mLastMotionY = mInitialTouchY + mTouchSlop * (dy < 0 ? -1 : 1);
                            }
                        }
                    } else { // For BOTH, GRID, and individual H/V modes
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                mLockedScrollAxis = SCROLL_ORIENTATION_NONE;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mInitialTouchY = mLastMotionY = (int) ev.getY();
                mInitialTouchX = mLastMotionX = (int) ev.getX();
                mLockedScrollAxis = SCROLL_ORIENTATION_NONE;
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mScrollState != SCROLL_STATE_DRAGGING) {
                    ...
                    final int yDiff = y - mInitialTouchY;
                    boolean startScroll = false;
                    ...
                    if (mScrollOrientation == SCROLL_ORIENTATION_HORIZONTAL_VERTICAL) {
                        ...
                        if (canScrollHorizontally && Math.abs(xDiff) > mTouchSlop) {
                            startScroll = true;
                        }
                        if (canScrollVertically && Math.abs(yDiff) > mTouchSlop) {
                            startScroll = true;
                        }
                    }
                ...
                if (mScrollState == SCROLL_STATE_DRAGGING) {
                    ...
                    if (mScrollOrientation == SCROLL_ORIENTATION_GRID) {
                        ...
                        if (mLockedScrollAxis == SCROLL_ORIENTATION_HORIZONTAL) {
                            scrollYDelta = 0;
                        } else if (mLockedScrollAxis == SCROLL_ORIENTATION_VERTICAL) {
                            scrollXDelta = 0;
                        }
                    }
                    ...
                    if (overScrollByCompat(scrollXDelta, scrollYDelta, getScrollX(), getScrollY(), rangeX, rangeY, 0, 0, true)) {
...
//                        mVelocityTracker.clear();
                    ...
                    if (dispatchNestedScroll(scrolledDeltaX, scrolledDeltaY, unconsumedX, unconsumedY, mScrollOffset)) {
                        ...
                        ensureGlows();
                        final int pulledToY = oldY + scrollYDelta;
                        final int pulledToX = oldX + scrollXDelta;
                        if (canScrollVertically()) {
                            if (pulledToY < 0) {
                                mEdgeGlowTop.onPull((float) scrollYDelta / getHeight(), MotionEventCompat.getX(ev, activePointerIndex) / getWidth());
                                if (!mEdgeGlowBottom.isFinished()) {
                                    mEdgeGlowBottom.onRelease();
                                }
                            } else if (pulledToY > rangeY) {
                                mEdgeGlowBottom.onPull((float) scrollYDelta / getHeight(), 1.f - MotionEventCompat.getX(ev, activePointerIndex) / getWidth());
                        ...
                        if (canScrollHorizontally()) {
                            if (pulledToX < 0) {
                                mEdgeGlowLeft.onPull((float) scrollXDelta / getWidth(), MotionEventCompat.getX(ev, activePointerIndex) / getHeight());
                                if (!mEdgeGlowLeft.isFinished()) {
                                    mEdgeGlowLeft.onRelease();
                                }
                            } else if (pulledToX > rangeX) {
                                mEdgeGlowRight.onPull((float) scrollXDelta / getWidth(), 1.f - MotionEventCompat.getX(ev, activePointerIndex) / getHeight());
